  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/8">
        <div>
          <div className="text-xs font-mono font-bold uppercase tracking-widest text-red-400 flex items-center gap-1.5">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight mt-1 flex items-center gap-2.5">
            <GradientText colors={["#FFFFFF", "#F87171", "#FB923C", "#FFFFFF"]}>
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-xs text-zinc-300 transition hover:text-white"
            title="Refresh Realtime Sync"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? "animate-spin text-blue-400" : "text-zinc-400"}`} />
            <span className="font-mono text-[11px]">{isRefreshing ? "Syncing..." : "Sync Live"}</span>
          </button>

          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-red-950/40 border border-red-800/40 text-xs text-red-300">
      ...
      {/* ========================================================================= */}
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-[#0e131f] via-[#090b11] to-[#08090d] border border-blue-500/30 p-5 sm:p-6 shadow-2xl shadow-blue-950/40">
        <BorderBeam size={250} duration={8} colorFrom="#38bdf8" colorTo="#3b82f6" />
        <Meteors number={8} />

        {/* Action Success Toast Banner */}
        {actionSuccessMessage && (
          <div className="mb-5 p-3.5 rounded-xl bg-emerald-950/70 border border-emerald-500/40 text-emerald-200 text-xs flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
              <span>{actionSuccessMessage.message}</span>
              {actionSuccessMessage.accessCode && (
                <span className="px-2 py-0.5 rounded bg-emerald-900/80 border border-emerald-400/50 text-white font-mono font-bold tracking-widest text-[11px]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-white/10 relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-blue-600/20 border border-blue-400/40 flex items-center justify-center text-blue-400 shadow-lg shadow-blue-500/20">
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-xl bg-black/40 border border-white/10">
            ...
            >
              ...
              <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                pendingCandidates.length > 0 ? "bg-blue-900 text-blue-200" : "bg-zinc-800 text-zinc-400"
              }`}>
            ...
            >
              ...
              <span>Claims</span>
              <span className={`text-[10px] font-mono px-1.5 py-0.2 rounded-full ${
                pendingQueue.length > 0 ? "bg-purple-900 text-purple-200" : "bg-zinc-800 text-zinc-400"
              }`}>
                {pendingQueue.length}
              </span>
            </button>
            ...
            <div>
              {pendingCandidates.length === 0 ? (
                <div className="py-8 text-center rounded-xl bg-zinc-950/40 border border-zinc-800/60 p-6 space-y-2">
                  <div className="w-10 h-10 rounded-full bg-emerald-950/40 border border-emerald-600/30 text-emerald-400 flex items-center justify-center mx-auto">
                    <ShieldCheck className="w-5 h-5" />
                  </div>
                  <div className="text-sm font-bold text-zinc-200">No Pending Candidate Requests</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="ascend-panel p-5 space-y-3 relative overflow-hidden border-blue-500/30 hover:border-blue-500/60 transition">
          <div className="flex items-center justify-between">
            ...
            {pendingCandidates.length > 0 && (
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/40 animate-pulse">
                {pendingCandidates.length} PENDING
              </span>
            )}
          </div>
          <div>
            <h3 className="text-sm font-bold text-white">Access Authorization Queue</h3>
            <p className="text-xs text-[#8B8B9A] mt-1 leading-relaxed">
        ...
        <div className="ascend-panel p-5 space-y-3 relative overflow-hidden border-purple-500/30 hover:border-purple-500/60 transition">
          <div className="flex items-center justify-between">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Verification Triage Queue</h3>
            <p className="text-xs text-[#8B8B9A] mt-1 leading-relaxed">
        ...
        <div className="ascend-panel p-5 space-y-3 relative overflow-hidden border-amber-500/30 hover:border-amber-500/60 transition">
          <div className="flex items-center justify-between">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Team Members Management</h3>
            <p className="text-xs text-[#8B8B9A] mt-1 leading-relaxed">
        ...
        <div className="ascend-panel p-5 space-y-3 relative overflow-hidden border-zinc-800 hover:border-zinc-700 transition">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">System Security & Audit</h3>
            <p className="text-xs text-[#8B8B9A] mt-1 leading-relaxed">
        ...
        {dataLoading && membersLeaderboard.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-zinc-800 text-[11px] font-mono text-zinc-400 uppercase tracking-wider">
                ...
                {membersLeaderboard.slice(0, 10).map((m, idx) => (
                  <tr key={m.id} className="hover:bg-zinc-900/40 transition">
                    <td className="py-3 px-3 text-center font-mono font-bold">
                      <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs ${
                        idx === 0 && m.points > 0
                          ...
                          : idx === 1 && m.points > 0
                          ? "text-zinc-300 bg-zinc-800/60 border border-zinc-600/50"
                          : idx === 2 && m.points > 0
                          ? "text-amber-600 bg-amber-950/40 border border-amber-800/40"
                          : "text-zinc-500"
                      }`}>
                        {idx + 1}
                      </span>
                    ...
                    <td className="py-3 px-3 text-right">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-950/40 border border-amber-600/30 text-amber-300 font-mono font-bold text-xs">
                        <Trophy className="w-3 h-3 text-amber-400" />
                        <span>{m.points}</span>
                        <span className="text-[10px] font-normal text-amber-400/80">pts</span>
                      </span>
                    </td>
                  </tr>
                ))}
