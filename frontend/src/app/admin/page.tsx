            ...
            <div>
              {pendingCandidates.length === 0 ? (
                ...
                <div className="space-y-3">
                  <div className="text-xs font-mono text-slate-600 dark:text-zinc-400 flex items-center justify-between">
                    <span>{pendingCandidates.length} candidate(s) awaiting verification</span>
                    <span className="text-[10px] text-blue-600 dark:text-blue-400 font-semibold">1-Click Fast Approval Enabled</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    {pendingCandidates.map((req) => (
                      <div
                        key={req.id}
                        className="rounded-xl bg-white dark:bg-zinc-950/70 border border-slate-200 dark:border-zinc-800 hover:border-blue-500/40 p-4 transition-all duration-200 space-y-3 relative group shadow-xs"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2.5">
                            <div className="w-9 h-9 rounded-lg bg-blue-100 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-500/30 text-blue-600 dark:text-blue-400 font-mono font-bold text-xs flex items-center justify-center shrink-0">
                              {req.name.slice(0, 2).toUpperCase()}
                            </div>
                            <div>
                              <div className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{req.name}</div>
                              <div className="text-[11px] font-mono text-slate-600 dark:text-zinc-400">{req.email}</div>
                            </div>
                          </div>
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-700/50 shrink-0 font-semibold">
                            PENDING
                          </span>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-mono">
                          {req.enrollment_number && (
                            <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-slate-700 dark:text-zinc-300">
                              Enr: {req.enrollment_number}
                            </span>
                          )}
                          {req.branch && (
                            <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-blue-700 dark:text-blue-300 font-semibold">
                              {req.branch} {req.section ? `(Sec ${req.section})` : ""}
                            </span>
                          )}
                          {req.department && (
                            <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 text-amber-800 dark:text-amber-300">
                              Dept: {req.department}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-200 dark:border-zinc-800/80">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-red-100 hover:bg-red-200 dark:bg-red-950/40 dark:hover:bg-red-900/50 border border-red-200 dark:border-red-800/50 text-red-700 dark:text-red-300 text-xs font-semibold transition disabled:opacity-50 flex items-center gap-1"
                          >
                            <X className="w-3.5 h-3.5" />
                            <span>Reject</span>
                          </button>
                          ...
                          <button
                            ...
                            disabled={processingId === req.id}
                            className="px-4 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition disabled:opacity-50 flex items-center gap-1.5 shadow-md shadow-emerald-600/30"
                          >
                            {processingId === req.id ? (
          ...
          {activeDeskTab === "achievements" && (
            <div>
              {pendingQueue.length === 0 ? (
                <div className="py-8 text-center rounded-xl bg-slate-50 dark:bg-zinc-950/40 border border-slate-200 dark:border-zinc-800/60 p-6 space-y-2">
                  <div className="w-10 h-10 rounded-full bg-purple-100 dark:bg-purple-950/40 border border-purple-300 dark:border-purple-600/30 text-purple-600 dark:text-purple-400 flex items-center justify-center mx-auto">
                    <CheckSquare className="w-5 h-5" />
                  </div>
                  <div className="text-sm font-bold text-slate-800 dark:text-zinc-200">Verification Queue Clear</div>
                ...
                <div className="space-y-3">
                  <div className="text-xs font-mono text-slate-600 dark:text-zinc-400 flex items-center justify-between">
                    ...
                    <Link href="/core/queue" className="text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 flex items-center gap-1 font-semibold">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    {pendingQueue.slice(0, 4).map((claim) => (
                      <div
                        key={claim.id}
                        className="rounded-xl bg-white dark:bg-zinc-950/70 border border-slate-200 dark:border-zinc-800 hover:border-purple-500/40 p-4 transition-all duration-200 space-y-3 shadow-xs"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="text-xs font-mono text-purple-600 dark:text-purple-400 uppercase tracking-wider font-semibold">
                              {claim.category_name || "ACHIEVEMENT CLAIM"}
                            </div>
                            <h4 className="text-sm font-bold text-slate-900 dark:text-white line-clamp-1 mt-0.5">{claim.title}</h4>
                            <div className="text-[11px] text-slate-600 dark:text-zinc-400 mt-0.5">By: {claim.member_name || "Team Member"}</div>
                          </div>
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-700/50 shrink-0 font-semibold">
                            PENDING REVIEW
                          </span>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-zinc-800/80">
                          <Link
                            ...
                            className="text-xs text-slate-600 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-zinc-200 font-mono flex items-center gap-1 font-medium"
                          >
                          ...
                          <button
                            ...
                            className="px-3.5 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold transition disabled:opacity-50 flex items-center gap-1.5 shadow-md shadow-purple-600/30"
                          >
