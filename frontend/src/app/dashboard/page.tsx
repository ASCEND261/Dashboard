  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      {realtime.hasScoreIncreased && (
        <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-500/50 text-emerald-900 dark:text-emerald-200 text-xs flex items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 shadow-xs">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0 animate-bounce" />
            <span className="font-bold text-emerald-950 dark:text-white">+{realtime.scoreDelta} PTS Awarded!</span>
            <span className="text-emerald-700 dark:text-emerald-300 text-[11px]">Real-time ledger score updated live.</span>
          </div>
          <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900 border border-emerald-300 dark:border-emerald-400/40 text-emerald-800 dark:text-emerald-200">
            REALTIME SYNC
          </span>
        </div>
      )}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="text-xs font-bold text-slate-900 dark:text-white tracking-wide">Team ASCEND</span>
              <span className={`text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full ${
                (user?.branch || "").includes("AI")
                  ? "bg-purple-100 text-purple-800 border border-purple-200 dark:bg-purple-950/80 dark:text-purple-300 dark:border-purple-800/60"
                  : (user?.branch || "").includes("VLSI")
                  ? "bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-950/80 dark:text-emerald-300 dark:border-emerald-800/60"
                  : (user?.branch || "").includes("CYBER")
                  ? "bg-red-100 text-red-800 border border-red-200 dark:bg-red-950/80 dark:text-red-300 dark:border-red-800/60"
                  : "bg-blue-100 text-blue-800 border border-blue-200 dark:bg-blue-950/80 dark:text-blue-300 dark:border-blue-800/60"
              }`}>
                {user?.branch || user?.department?.code || user?.department_unit || "MEMBER"}
              </span>
              {user?.section && (
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 border border-slate-200 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-300">
                  Sec {user.section}
                </span>
              )}
              {user?.enrollment_number && (
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-400 hidden sm:inline-block">
                  Enr: {user.enrollment_number}
                </span>
              )}
              <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-400 dark:border-emerald-800/40 flex items-center gap-1.5 ml-auto">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" />
                <span>{realtime.teamScore} PTS LIVE</span>
              </span>
            </div>

            <div className="text-[11px] font-mono uppercase tracking-widest text-slate-500 dark:text-zinc-400 flex items-center gap-1.5 mb-1 font-semibold">
              <Sparkles className="w-3 h-3 text-blue-500" />
              <span>{getISTGreeting(user?.name)}</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
              <GradientText colors={["#0F172A", "#2563EB", "#7C3AED", "#0F172A"]}>
                ASCEND is climbing.
              </GradientText>
          ...
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-3 sm:py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm sm:text-xs font-bold tracking-wide transition shadow-md shadow-blue-600/25 active:scale-[0.98] shrink-0 w-full sm:w-auto hover:scale-[1.02]"
          >
      ...
      <div className="space-y-2">
        <div className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-gray-400 flex items-center justify-between px-1">
          <span className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-500" />
            <span>Quick-Start Claim Presets</span>
          </span>
          <span className="text-[10px] text-slate-500 dark:text-zinc-400 hidden sm:block">Deterministic Scoring Catalog</span>
        </div>
      ...
      <div className="flex items-center gap-1.5 p-1 rounded-xl bg-slate-100 dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 overflow-x-auto shadow-xs">
        <button
          onClick={() => handleTabChange("overview")}
          className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-medium transition shrink-0 ${
            activeTab === "overview"
              ? "bg-white dark:bg-zinc-800 text-slate-900 dark:text-white border border-slate-200/90 dark:border-zinc-800 shadow-xs font-bold"
              : "text-slate-600 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/60 dark:hover:bg-[#141418]"
          }`}
        >
        ...
        >
          ...
          {currentSubmissions.length > 0 && (
            <span className="px-1.5 py-0.2 rounded bg-slate-200/80 dark:bg-[#141418] text-slate-700 dark:text-gray-300 font-mono text-[10px] border border-slate-300 dark:border-zinc-800">
      ...
      <PageTransition transitionKey={activeTab}>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <SpotlightCard className="p-4 sm:p-5 active:scale-[0.98] transition-transform">
                <div className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-gray-400">
                  Overall Rank
                </div>
                <div className="text-2xl sm:text-3xl font-extrabold font-mono text-slate-900 dark:text-white mt-1">
                  {currentProgress?.rank || "#01"}
                </div>
                <div className="text-[11px] text-slate-500 dark:text-gray-400 mt-1 flex items-center gap-1 font-mono">
              ...
              <SpotlightCard className="p-4 sm:p-5 active:scale-[0.98] transition-transform">
                <div className="text-[10px] font-mono font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">
                  Team Points
                </div>
                <div className="text-2xl sm:text-3xl font-extrabold font-mono text-slate-900 dark:text-zinc-100 mt-1">
              ...
              <SpotlightCard className="p-4 sm:p-5 active:scale-[0.98] transition-transform">
                ...
                <div className="text-[11px] text-slate-500 dark:text-gray-400 mt-1 font-mono">100% proof certified</div>
              </SpotlightCard>
              ...
              <SpotlightCard className="p-4 sm:p-5 active:scale-[0.98] transition-transform">
                ...
                <div className="text-[11px] text-slate-500 dark:text-gray-400 mt-1 font-mono">Real-time velocity</div>
              </SpotlightCard>
            </div>

            {/* Rubric Progress Card */}
            <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 space-y-4 shadow-xs">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-slate-900 dark:text-white">Sprint Target Execution</h3>
            ...
            <div className="p-5 rounded-2xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 space-y-4 shadow-xs">
              <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                  <Activity className="w-4 h-4 text-blue-500" />
                  <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-wide">Recent Team-Level Velocity</h2>
                </div>
                <span className="text-[11px] font-mono text-slate-500 dark:text-gray-400">Aggregated Stream</span>
              </div>

              <div className="divide-y divide-slate-100 dark:divide-zinc-800/80">
                {(!currentProgress?.recent_activity || currentProgress.recent_activity.length === 0) ? (
                  <div className="py-6 text-center text-xs text-slate-500 dark:text-zinc-500 font-mono">
                  ...
                  currentProgress.recent_activity.map((act: any) => (
                    <div key={act.id} className="py-3 flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="text-xs font-semibold text-slate-900 dark:text-gray-200">{act.title}</div>
                          <div className="text-[11px] text-emerald-600 dark:text-emerald-400 font-mono mt-0.5">{act.impact}</div>
                        </div>
                      </div>
                      <div className="text-[10px] text-slate-500 dark:text-gray-400 shrink-0 font-mono">{act.timestamp}</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="p-5 rounded-2xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 space-y-4 shadow-xs">
              <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-wide">Recent Submission History</h2>
                </div>
                <button
                  onClick={() => handleTabChange("history")}
                  className="text-xs text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-200 font-medium flex items-center gap-1 transition"
                >
                  ...
                  {currentSubmissions.slice(0, 3).map((item) => (
                    <div key={item.id} className="py-3 flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-[#141418] text-slate-700 dark:text-gray-300 border border-slate-200 dark:border-zinc-800">
                            {item.category}
                          </span>
                          <span className="text-xs font-bold text-slate-900 dark:text-white truncate max-w-xs">{item.title}</span>
                      ...
                      <div className="text-right shrink-0">
                        {item.points_awarded !== null ? (
                          <div className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400">+{item.points_awarded} pts</div>
                        ) : (
                          <div className="text-xs font-mono text-slate-500 dark:text-gray-400">Under Review</div>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-4 rounded-xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 shadow-xs">
                ...
                <div className="text-2xl font-black font-mono text-slate-900 dark:text-white mt-1">{currentSubmissions.length}</div>
                <div className="text-[10px] text-slate-400 dark:text-gray-500 font-mono">All-time claims</div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 shadow-xs">
                <div className="text-[10px] font-mono uppercase text-slate-500 dark:text-zinc-400 font-semibold">Points Minted</div>
                <div className="text-2xl font-black font-mono text-slate-900 dark:text-zinc-100 mt-1">
                  {currentSubmissions
                    .filter((s) => s.status === "VERIFIED")
                    .reduce((acc, s) => acc + (s.points_awarded || 0), 0)}
                </div>
                <div className="text-[10px] text-slate-400 dark:text-zinc-400/70 font-mono">TSJ-2026-v1 Rulebook</div>
              </div>
              <div className="p-4 rounded-xl bg-white dark:bg-[#0E0E11] border border-amber-200 dark:border-amber-500/20 shadow-xs">
                <div className="text-[10px] font-mono uppercase text-amber-600 dark:text-amber-400 font-semibold">Under Review</div>
                <div className="text-2xl font-black font-mono text-amber-600 dark:text-amber-300 mt-1">
                  {currentSubmissions.filter((s) => s.status === "SUBMITTED" || s.status === "UNDER_REVIEW" || s.status === "NEEDS_MORE_PROOF").length}
                </div>
                <div className="text-[10px] text-amber-600/70 dark:text-amber-500/70 font-mono">In verifier triage</div>
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 overflow-hidden shadow-xs">
              <div className="px-5 py-3.5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50 dark:bg-[#141418]">
                ...
                <Link
                  href="/dashboard/submit"
                  className="text-xs text-blue-600 dark:text-zinc-400 hover:text-blue-700 dark:hover:text-zinc-200 flex items-center gap-1 font-semibold"
                >
              ...
              {submissions
                ...
                .map((item) => (
                  <div key={item.id} className="p-5 border-b border-slate-100 dark:border-zinc-800 hover:bg-slate-50/70 dark:hover:bg-[#141418]/50 transition space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2 font-mono">
                        ...
                        <span className="text-[11px] text-slate-500 dark:text-gray-400">Activity: {item.date_of_activity}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        {getStatusBadge(item.status)}
                      </div>
                    ...
                    {item.proof_request_reason && (
                      <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-500/30 text-xs text-amber-800 dark:text-amber-200 flex items-start gap-2">
                        ...
                        <div>
                          <span className="font-bold text-amber-900 dark:text-amber-300 font-mono">More Proof Required: </span>
        ...
        {activeTab === "achievements" && (
          <div className="space-y-4">
            <div className="rounded-2xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 overflow-hidden shadow-xs">
              <div className="px-5 py-3.5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50 dark:bg-[#141418]">
                <div className="text-xs font-semibold text-slate-700 dark:text-gray-300 uppercase tracking-wider font-mono">
                  My Submitted Achievements ({currentSubmissions.length})
                </div>
                <Link
                  href="/dashboard/submit"
                  className="text-xs font-medium text-blue-600 dark:text-zinc-400 hover:text-blue-700 dark:hover:text-zinc-200 flex items-center gap-1 font-semibold"
                >
              ...
              {currentSubmissions.length === 0 ? (
                <div className="p-12 text-center">
                  <Inbox className="w-8 h-8 text-slate-400 dark:text-gray-500 mx-auto mb-2" />
                  <p className="text-xs text-slate-500 dark:text-gray-400">No achievements recorded yet.</p>
                  <Link
                    href="/dashboard/submit"
                    className="inline-block mt-3 text-xs text-blue-600 dark:text-zinc-400 hover:underline font-semibold"
                  >
                ...
                <div className="divide-y divide-slate-100 dark:divide-zinc-800/80">
                  {currentSubmissions.map((item) => (
                    <div key={item.id} className="p-4 hover:bg-slate-50/70 dark:hover:bg-[#141418]/60 transition flex flex-col md:flex-row md:items-center justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-[#141418] text-slate-700 dark:text-gray-300 border border-slate-200 dark:border-zinc-800">
                            {item.category}
                          </span>
                          <span className="text-xs font-mono text-slate-500 dark:text-gray-400">{item.date_of_activity}</span>
                          {getStatusBadge(item.status)}
                        </div>
                        <h3 className="text-sm font-bold text-slate-900 dark:text-gray-100">{item.title}</h3>
                        <p className="text-xs text-slate-600 dark:text-gray-400">{item.description}</p>
                      </div>

                      <div className="flex items-center gap-4 shrink-0">
                        <div className="text-right">
                          <div className="text-sm font-mono font-bold text-slate-900 dark:text-gray-200">
                            {item.points_awarded !== null ? (
                              <span className="text-emerald-600 dark:text-emerald-400 font-semibold">+{item.points_awarded} pts</span>
                            ) : (
                              <span className="text-slate-400 dark:text-gray-400 text-xs">Pending Review</span>
                            )}
                          </div>
                        </div>
                        ...
                        {item.proof_documents && item.proof_documents.length > 0 && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200/80 dark:bg-[#141418] dark:hover:bg-[#18181B] border border-slate-200 dark:border-zinc-800 text-xs text-slate-700 dark:text-gray-300 hover:text-slate-900 dark:hover:text-white transition flex items-center gap-1.5 shadow-xs"
                          >
                            <FileText className="w-3.5 h-3.5 text-blue-500" />
                            <span>Proof</span>
                          </button>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-white dark:bg-[#0E0E11] border border-slate-200 dark:border-zinc-800 overflow-hidden shadow-xs">
              <div className="px-5 py-3.5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50 dark:bg-[#141418]">
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-gray-400 font-mono">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between px-5 py-3.5 transition ${
                        isAscend
                          ? "bg-blue-50/70 dark:bg-zinc-850 border-l-2 border-l-blue-600"
                          : "hover:bg-slate-50/70 dark:hover:bg-[#141418]/50"
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center font-mono font-bold text-xs ${
                            team.rank === "01"
                              ? "bg-amber-100 text-amber-800 border border-amber-300 shadow-xs dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-500/30"
                              : team.rank === "02"
                              ? "bg-slate-100 text-slate-800 border border-slate-300 dark:bg-zinc-800 dark:text-zinc-200 dark:border-zinc-800"
                              : "bg-slate-50 text-slate-600 border border-slate-200 dark:bg-[#141418] dark:text-gray-400 dark:border-zinc-800"
                          }`}
                        >
                          {team.rank}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <span className={`text-sm font-bold ${isAscend ? "text-slate-900 dark:text-white" : "text-slate-800 dark:text-gray-300"}`}>
                              {team.name}
                            </span>
                            {isAscend && (
                          ...
                          <div className="text-[11px] text-slate-500 dark:text-gray-400 font-mono">Team ID: {team.team_id}</div>
                        </div>
                      </div>

                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <div className="text-base font-black font-mono text-slate-900 dark:text-gray-100">
                            {team.points.toLocaleString()}
                          </div>
                          <div className="text-[10px] text-slate-500 dark:text-gray-400 uppercase tracking-wider font-mono">
                            Official Pts
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <SpotlightCard className="p-5">
                <div className="text-xs font-mono font-bold uppercase text-slate-500 dark:text-zinc-400">Sprint Target</div>
                <div className="text-2xl font-mono font-black text-slate-900 dark:text-white mt-1">2,000 Pts</div>
                <p className="text-xs text-slate-600 dark:text-gray-400 mt-1">Target for top tier cohort placement</p>
              </SpotlightCard>
              <SpotlightCard className="p-5">
                <div className="text-xs font-mono font-bold uppercase text-slate-700 dark:text-zinc-200">Velocity</div>
                <div className="text-2xl font-mono font-black text-slate-900 dark:text-white mt-1">+180 pts/wk</div>
                <p className="text-xs text-slate-600 dark:text-gray-400 mt-1">Pacing 14% ahead of average team trajectory</p>
              </SpotlightCard>
              <SpotlightCard className="p-5">
                <div className="text-xs font-mono font-bold uppercase text-emerald-600 dark:text-emerald-400">Audit Status</div>
                <div className="text-2xl font-mono font-black text-emerald-600 dark:text-emerald-400 mt-1">100% Certified</div>
                <p className="text-xs text-slate-600 dark:text-gray-400 mt-1">All verified points backed by uploaded proof</p>
              </SpotlightCard>
            </div>
          </div>
        )}
