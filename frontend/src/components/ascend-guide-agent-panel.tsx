...
export default function AscendGuideAgentPanel({ isOpen, onClose }: AscendGuideAgentPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      {/* Drawer Panel */}
      <div className="relative w-full max-w-lg h-full sm:h-[92vh] sm:mr-4 sm:rounded-2xl flex flex-col bg-[#0A0C10] border-l sm:border border-zinc-800/80 shadow-2xl overflow-hidden z-10 animate-in slide-in-from-right-4 duration-200">
        {/* Top Accent Light Bar */}
        <div className="h-[2px] w-full bg-gradient-to-r from-transparent via-blue-500/60 to-transparent" />

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800/80 bg-[#0D0F14]/90 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-950/60 text-blue-400 border border-blue-800/40 font-mono font-medium">
                  Authoritative
                </span>
              </div>
              <p className="text-[11px] text-zinc-400">Your guide to how ASCEND works</p>
            </div>
          </div>
        ...
        <div className="px-5 py-2 bg-[#090A0D] border-b border-zinc-800/50 flex items-center justify-between text-[10px] font-mono text-zinc-400">
          <span className="flex items-center gap-1.5 text-zinc-300">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            <span>AI assists; Rust decides/enforces.</span>
          </span>
          <span className="text-zinc-500">Ruleset TSJ-2026-v1</span>
        </div>

        {/* Message Thread */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 bg-[#07080A]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {STARTER_QUESTIONS.map((q) => (
                  <button
                    ...
                    className="text-left px-3 py-2 rounded-xl bg-[#0F1117] hover:bg-[#161922] border border-zinc-800/80 hover:border-blue-500/40 text-zinc-300 hover:text-white text-xs transition font-mono leading-snug"
                  >
                    "{q}"
                  </button>
                ))}
              ...
              <div
                className={`max-w-[90%] rounded-xl px-4 py-3 text-xs leading-relaxed ${
                  m.role === "user"
                    ...
                    : "bg-[#0E1017] text-zinc-200 border border-zinc-800 shadow-md font-sans"
                }`}
              >
                ...
                {m.knowledge_references && m.knowledge_references.length > 0 && (
                  <div className="mt-3 pt-2 border-t border-zinc-800/80 flex flex-wrap items-center gap-1.5 font-mono">
                    ...
                    {m.knowledge_references.map((ref, idx) => (
                      <span
                        key={idx}
                        className="text-[10px] px-2 py-0.5 rounded bg-zinc-900 text-blue-400 border border-zinc-800"
                      >
                        {ref}
                      </span>
                    ))}
                ...
                <div className="flex flex-wrap gap-1.5 mt-2 ml-1">
                  {m.suggested_actions.map((act, idx) => (
                    <button
                      ...
                      className="text-[10px] px-2.5 py-1 rounded-lg bg-zinc-900/80 hover:bg-zinc-800 border border-zinc-800/80 text-zinc-400 hover:text-white transition font-mono flex items-center gap-1"
                    >
                      <span>{act}</span>
                      <ArrowUp className="w-2.5 h-2.5 rotate-45 text-blue-400" />
                    </button>
                  ))}
                </div>
              )}
        ...
        {/* Input Bar */}
        <div className="p-3 sm:p-4 border-t border-zinc-800/80 bg-[#0B0C10]">
          <div className="relative flex items-center">
            <input
              ...
              disabled={loading}
              className="w-full bg-[#0E1015] border border-zinc-800 focus:border-blue-500 rounded-xl pl-4 pr-12 py-2.5 text-xs text-white placeholder-zinc-500 focus:outline-none transition shadow-inner font-mono"
            />
            <button
              onClick={() => handleSend()}
