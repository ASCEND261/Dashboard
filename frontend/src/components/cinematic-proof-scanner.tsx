...
export default function CinematicProofScanner({
  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-[#07080A] rounded-3xl border border-zinc-800 p-5 sm:p-7 shadow-2xl flex flex-col items-center relative overflow-hidden">
      
      {/* Interactive Scenario Switcher Pills */}
      <div className="flex items-center gap-1.5 p-1 bg-zinc-900/90 border border-zinc-800 rounded-xl mb-6 text-[10px] font-mono">
        <button
          onClick={() => runScan("valid_winner")}
          className={`px-3 py-1 rounded-lg transition font-medium ${
            scenario === "valid_winner" ? "bg-blue-600 text-white shadow" : "text-zinc-400 hover:text-zinc-200"
          }`}
        >
          ① AutoVerify (+50)
        </button>
        <button
          ...
          className={`px-3 py-1 rounded-lg transition font-medium ${
            scenario === "ambiguous_dsa" ? "bg-amber-600 text-white shadow" : "text-zinc-400 hover:text-zinc-200"
          }`}
        >
          ② Ambiguous (0 Core)
        </button>
        <button
          ...
          className={`px-3 py-1 rounded-lg transition font-medium ${
            scenario === "invalid_unrelated" ? "bg-red-600 text-white shadow" : "text-zinc-400 hover:text-zinc-200"
          }`}
        >
          ③ Invalid (0 Rej)
        </button>
      ...
      <div className="relative w-full max-w-sm h-48 rounded-2xl bg-[#0D0F12] border border-zinc-700/80 p-4 flex flex-col justify-between overflow-hidden shadow-inner group">
        
        {/* Electric Blue Scanning Laser Beam */}
        {stage === "SCANNING" && (
          <div className="absolute left-0 right-0 h-1 bg-gradient-to-r from-transparent via-blue-400 to-transparent shadow-[0_0_15px_#5B8CFF] animate-[bounce_2s_infinite] pointer-events-none z-20" />
        )}
        ...
        <div className="flex items-center justify-between text-xs text-zinc-400 font-mono">
          ...
          <span className="text-[10px] text-zinc-500 uppercase">SHA-256</span>
        </div>
        ...
        <div className="flex items-center justify-between text-[10px] font-mono pt-2 border-t border-zinc-800/80">
          ...
          <span className="text-blue-400 font-bold">RUST EVIDENCE GATE</span>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="font-bold text-[10px] tracking-wider text-zinc-400">{d.name}</span>
                <span className="text-[11px] text-zinc-300 truncate max-w-[190px]">{d.detail}</span>
              </div>

              <div>
                {d.status === "pass" && <CheckCircle2 className="w-4 h-4 text-emerald-400 stroke-[2.5]" />}
                {d.status === "warn" && <AlertCircle className="w-4 h-4 text-amber-400 stroke-[2.5]" />}
                {d.status === "fail" && <XCircle className="w-4 h-4 text-red-400 stroke-[2.5]" />}
                {d.status === "pending" && <div className="w-2 h-2 rounded-full bg-zinc-700" />}
              </div>
            </div>
          );
        })}
      ...
      {stage === "COMPLETE" && (
        <div className="w-full max-w-sm mt-6 animate-in fade-in zoom-in-95 duration-300">
          {scenario === "valid_winner" && (
            <div className="p-4 rounded-2xl bg-blue-950/20 border border-blue-600/40 text-center space-y-2">
              <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-emerald-400 bg-emerald-950/40 px-2.5 py-0.5 rounded-md border border-emerald-800/40">
              ...
              <p className="text-[11px] font-mono text-zinc-400">
                RULE MATCHED: <span className="text-blue-400">EXTERNAL_HACKATHON_1ST</span> &middot; LEDGER COMMITTED
              </p>
            </div>
          )}

          {scenario === "ambiguous_dsa" && (
            <div className="p-4 rounded-2xl bg-amber-950/20 border border-amber-600/40 text-center space-y-2">
              <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-amber-400 bg-amber-950/40 px-2.5 py-0.5 rounded-md border border-amber-800/40">
          ...
          {scenario === "invalid_unrelated" && (
            <div className="p-4 rounded-2xl bg-red-950/20 border border-red-600/40 text-center space-y-2">
              <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-red-400 bg-red-950/40 px-2.5 py-0.5 rounded-md border border-red-800/40">
          ...
          <button
            ...
            className="w-full mt-3 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-white text-xs font-mono flex items-center justify-center gap-1.5 transition"
          >
